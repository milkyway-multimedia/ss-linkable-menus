...
use Milkyway\SS\GridFieldUtils\AddExistingPicker;
use Milkyway\SS\GridFieldUtils\AddNewInlineExtended;
use Milkyway\SS\GridFieldUtils\MinorActionsHolder;

use DataList;
use Milkyway\SS\LinkableMenus\GridField\CopyChildrenFromCurrentPage;
use Milkyway\SS\LinkableMenus\GridField\CopyChildrenFromRoot;
...
class LinkThatCanActAsMenu extends \DataExtension
{
		...
		$editableColumns = [
			'AltTitle' => [
				...
				'callback' => function ($record, $col) {
					return \TextField::create($col,
						_t('Linkable.MENU_LABEL', 'Menu Label'))->setAttribute('placeholder', $record->Title);
				},
			],
			'LinkType' => [
			...
			'LoggedInDisplay' => [
				'title' => _t('Linkable.DISPLAY', 'Display'),
				'callback' => function ($record, $col) {
					return \DropdownField::create($col, $col, [
						'Always' => _t('ALWAYS', 'Always'),
						'LoggedIn' => _t('LOGGED_IN', 'Logged In'),
						'NotLoggedIn' => _t('NOT_LOGGED_IN', 'Not Logged In'),
					]);
				},
			],
		];
	...
	public function allowLinkToActAsMenu(
		$fields,
		$form = null,
		$relation = 'MenuLinks',
		$parent = null,
		...
		if (!($typeField = $fields->fieldByName('Root.Main.Type')) || !($typeField instanceof \TabbedSelectionGroup)) {
			$fields->addFieldsToTab('Root.Main', [
				...
				$fieldsForForm[] = \FormMessageField::create('MSG-LINK_TO',
					_t('Link.MSG-LINK_TO', 'When as user clicks on this link in the menu, where does it go?'),
					'info')->cms(),
			], 'Type');
		}
		...
		if ($item->exists()) {
			$fieldsForForm[] = $links = \GridField::create($relation, _t('Link.LINKS', 'Links'), $item->$relation(),
				$config = \GridFieldConfig_RecordEditor::create()
					...
					->addComponents(
						...
						new MinorActionsHolder,
						$addNewExtended = new AddNewInlineExtended('actions-buttons-before-left')
					)
			);
			...
			foreach ($model->hasOne() as $hasOneName => $hasOneClass) {
				$config->addComponent(
					(new AddExistingPicker('actions-buttons-before-left'))
						->setTitle(_t('LinkableMenus.LINK_TO_HAS_ONE', 'Link to a {item}',
							[
								'item' => singleton($hasOneClass)->i18n_singular_name(),
							]
						))
						->setSearchList(DataList::create($hasOneClass))
						->setUrlSegment('add-a-' . strtolower($hasOneName))
						->setAddHandler(function ($request, $grid, $button, $acquireItems) use (
							$model,
							$hasOneClass,
							$hasOneName
						) {
						...
						->setUndoHandler(function ($request, $grid, $button, $acquireItems) use ($model, $hasOneName) {
							...
							foreach ($items as $item) {
								if ($link = $grid->getList()->sort('Created DESC')->filter($hasOneName . 'ID',
									$item->ID)->first()
								) {
									$link->delete();
								}
			...
			if ($item && ($item instanceof \LinkableMenu || $item->Type == 'SiteTree')) {
				...
				$config->addComponents(
					(new AddExistingPicker('actions-buttons-before-left'))
						->setTitle(_t('LinkableMenus.COPY_CHILDREN_FROM', 'Copy children from other {item}',
							[
								'item' => singleton('SiteTree')->i18n_singular_name(),
							]
						))
						->setSearchList(DataList::create('SiteTree'))
						->setUrlSegment('copy-links-from-sitetree')
						->setAddHandler(function ($request, $grid, $button, $acquireItems) use (
							$model
						) {
							$items = $acquireItems($request);
						...
						->setUndoHandler(function ($request, $grid, $button, $acquireItems) use ($model) {
							...
							foreach ($items as $item) {
								if ($links = $grid->getList()->Relation('SiteTree')->sort('Created DESC')->filter('ParentID',
									$item->ID)
								) {
									$links->delete();
								}
			...
			$fieldsForForm[] = $links = \FormMessageField::create('ERR-SAVE_FIRST',
				_t('LinkableMenus.SAVE_FIRST', 'Please save before adding any sub-menu links'), 'warning')->cms();
		}
		...
		$this->owner->extend('updateLinkThatCanActAsMenuFields', $fields, $form, $relation, $parent, $controller,
			$item);
	}
	...
	{
		if (!singleton('LinkableMenu')->checkIfHasGlobalMenuPermission($member)) {
			return false;
		}

		if (singleton('SiteTree')->{__FUNCTION__}($member)) {
			return true;
		}
	}
	...
	public function canViewInMenu($member = null)
	{
		switch ($this->owner->LoggedInDisplay) {
			case 'LoggedIn':
	...
	protected function updateFields(\FieldList $fields)
	{
		...
		$types = [
			\SelectionGroup_Item::create('', \CompositeField::create(),
				_t('Linkable.TYPENONE', 'None'))->setName('Type_None'),
		];
	...
	protected function clearFieldList(\FieldList $fields)
	{
		foreach ($fields as $field) {
			...
			if (!$field->Name || !in_array($field->Name, [
					'Type',
					'URL',
					'Email',
					'File',
		...
		switch ($type) {
			case 'URL':
				return \CompositeField::create(
					...
					\CheckboxField::create('OpenInNewWindow',
			...
			case 'File':
				return \CompositeField::create(
					...
					\CheckboxField::create('File_OpenInNewWindow',
						_t('Linkable.OpenInNewWindow', 'Open link in a new window'))
				);
			case 'SiteTree':
				return \CompositeField::create(
					...
					\CheckboxField::create('SiteTree_OpenInNewWindow',
						_t('Linkable.OpenInNewWindow', 'Open link in a new window'))
				);
			default:
				return \CompositeField::create();
